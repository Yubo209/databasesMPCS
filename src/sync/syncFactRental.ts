...
export async function syncFactRental() {
  ...
  try {
    ...
    const state = await syncRepo.findOneBy({ table_name: "rental" });
    const marker = state?.last_marker ?? null;

    const rentals = !marker
      ? await rentalRepo.find()
      : await rentalRepo
          .createQueryBuilder("r")
          .where("r.last_update > :marker", { marker })
          .getMany();
    ...
    await factRepo.upsert(rows, ["rental_id"]);
